...
| `ItemKind` 14 tunique / 15 manteau (`COUNT` = 16 à l'époque, 19 depuis le métal), `EventKind::ItemCrafted = 20` (`arg` = genre) | `pawn_apparel(id)` (−1 si aucun) ; fabrication par `SetCraftTarget` |
...
| `Command::SetGoodwill { values: [i32; 3] }` (réputation de départ imposée par le monde, bornée, sans événement ; l'hôte l'émet après `SetDifficulty`, jamais en solo) | `set_goodwill(a, b, c)`, `encode_set_goodwill(a, b, c)` ; le serveur envoie `start.goodwill` / `snapshot.goodwill`, le client remonte `goodwill_report` |
...
| `biome::Biome` (0 océan, 1 banquise, 2 toundra, 3 forêt boréale, 4 forêt tempérée, 5 prairie, 6 désert, 7 savane, 8 jungle, 9 montagne : alignés sur `packages/world/src/biomes.ts`), fixé à la création (`Sim::new_in_biome`, pas de `Command::SetBiome` ; `new` = tempéré), `Terrain::Snow = 8` | `WasmSim.new_in_biome(seed, w, h, biome)`, `biome()` ; `TERRAIN.Snow`, `TERRAIN_COLORS[8]` ; le serveur devra fournir `start.biome` / `snapshot.biome` et `startGame` le passer au constructeur |
